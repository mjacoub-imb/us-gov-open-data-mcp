...
- **The deployed tool surface is a fraction of the codebase, by configuration.** The `Dockerfile` pins `TOOL_MODE=grouped`, a 14-module `MODULES=` allowlist, and a `FACADES_EXCLUDE=` list trimming `congress` to `congress_bills` — together a fraction of the preamble against ~418K for all 43 modules ungrouped. This is scoped to one consumer's research needs (PE diligence: utility/infrastructure/energy-services), *not* a judgment about which modules are worth keeping; the other 29 remain in the image, fully tested, one env-var edit away. Railway variables override the `Dockerfile` defaults, so check both when the deployed surface doesn't match what the file says.
...
The per-domain client pool (`portalClients`) is capped at `MAX_PORTAL_CLIENTS` (100 — well above the curated ~25 portals, so ordinary usage never evicts). Without the cap, `SOCRATA_ALLOW_ANY_PORTAL` let a caller varying `domain` grow the pool — and, via `createClient`'s per-`name` disk-cache namespace, a same-shaped `cache.json` namespace — without bound. Eviction calls the evicted client's `clearCache()` before dropping it, so the disk-cache namespace is actually freed rather than outliving the pool slot it came from.
...
A facade's own description is the primary routing signal once per-operation schemas are gone, built from `firstSentence(mod.description, 300)`. That function requires a lowercase/digit/closing-bracket immediately before the sentence-ending period — plain "split on '. '" isn't enough, because several module descriptions open with an abbreviation like "U.S." or "U.K.", where the character before the *second* period is itself uppercase, same shape as any real sentence start. Naive splitting there produced facade descriptions that were just "U.S." (bea, cdc, uspto) with the actual subject matter silently dropped. `tests/facade.test.ts` pins both the abbreviation case and that a real trailing sentence still gets found.
...
`MODULES=` is all-or-nothing per module, which is too coarse once a deployment only wants part of a split module — the Railway instance tracks legislation via `congress_bills` but has no use for members, committees, nominations or the Congressional Record, and loading `congress` drags in all five facades (~22K chars). `FACADES=` (keep only these) and `FACADES_EXCLUDE=` (drop these) filter `facadeGroups` after `planFacades`, via `applyFacadeFilter` (`src/server/facade.ts`, unit-tested in `tests/facade.test.ts`). Both are grouped-mode only and mutually exclusive; supplying both, using either in `full` mode, naming a facade that doesn't exist, or excluding everything all `process.exit(1)` rather than quietly serving a surface the operator didn't ask for. So does giving `--facades`/`--facades-exclude` with no value at all — the flag-parsing code checks for that explicitly, since a bare `get(flag)` lookup can't tell "flag absent" apart from "flag present but nothing followed it," and the latter used to silently apply no filter.

**Filtering `facadeGroups` is not enough on its own — the instructions have to know about it too.** A module's `workflow`/`tips`/`crossRef` text names specific operations by their original tool name (that's the whole point of the "operation names are byte-for-byte" invariant above), and that text is unconditional — every module gets its full text regardless of which of its facades survived. If `FACADES_EXCLUDE` drops the facade one of those named operations lives in, the text still tells the model to call it, but no registered tool can reach it anymore except `code_mode`. This was a real, verified bug: with the shipped `FACADES_EXCLUDE`, the model was told to call `congress_house_votes` and `congress_hearings` as if `congress_bills` (the one surviving facade) exposed them, got a schema-validation error listing the 21 operations it actually does expose, and had no textual cue that the fix was `code_mode(tool="congress_house_votes", ...)` instead.

Fixed via `exposedOperationsByModule` (`src/server/facade.ts`): computed from the *post-filter* `facadeGroups`, so `instructions.ts` knows exactly which of a module's real operation names are reachable through a currently-registered facade. For each module, `buildInstructions` now appends a line naming whichever of its tools aren't in that set, pointed at `code_mode` — printed once per module, so every later `workflow`/`tips`/`crossRef` mention of one of those names is already covered by the time the model reads it, without needing to parse or rewrite any of that free-text metadata. The degenerate case — every one of a module's facades excluded, so it has no entry in `facadesByModule` at all — used to silently fall back to the `full`-mode line (`Tools: <every raw tool name>`), which read as "these are direct tools" when literally none of them were registered as anything; that fallback is now its own explicit case, tested in `tests/instructions-builder.test.ts`, and the same file's "full mode is unaffected" test pins that none of this changes what `full` mode already did correctly.

Two other facts still matter. The filter runs **before** `facadeNamesByModule`/`exposedOperationsByModule`, so both the `Tools:` line and the hidden-operations note reflect what's actually registered. And the filter does **not** touch `allToolMap`, so dropped operations stay reachable through `code_mode` by their original name at zero `tools/list` cost; "dropped" here means hidden from the tool list, not removed — that's what makes the fix above sufficient (naming the gap) rather than needing to also disable the workflow/tips/crossRef text.
